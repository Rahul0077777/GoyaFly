...
const { getAllPackages, createPackage, updatePackage, deletePackage } = require('../controllers/visa.controller');
const { protectAdmin } = require('../middlewares/adminAuth.middleware');

// Multer config for visa package images
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        cb(null, 'uploads/');
    },
    filename: (req, file, cb) => {
        cb(null, `visa_${Date.now()}_${Math.round(Math.random() * 1000)}${path.extname(file.originalname)}`);
    }
});
